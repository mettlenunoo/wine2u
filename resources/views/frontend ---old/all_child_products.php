...
<?php
   ...
   $delete_product = preg_replace('#[^0-9]#', '', $_GET['delete_product']);
    ...
    $sql = "DELETE FROM product_child_tb WHERE id = :delete_product";
    $statement = $conn->prepare($sql);
    $ToDelete = $delete_product;
    $statement->bindValue(':delete_product', $ToDelete);
    $delete = $statement->execute();
    echo "<script> window.location.replace('all_child_products.php') </script>";
        }
     ...
     <?php require_once("includes/backend_side_bar.php");?>
            ...
            <main id="main-container">
                <div class="content">
                    <div class="row items-push">
                        <div class="col-sm-12">
                            <h1 class="page-heading">
                               All Child Products<small> </small>
                            </h1>
                        </div>
                    ...
                    $sql = "SELECT * from product_child_tb  ORDER BY date DESC, id DESC";
                    $q = $conn->query($sql);
...
?>
     ...
     <th class="text-center">Actions</th>
       ...
       <?php while ($row = $q->fetch()): ?>
                     <tr>
   <!-- <td class="text-center"><img src="" height="50px" width="50px"> </td> -->
   ...
   <td class="text-center"><?= $row['stock'];?></td>
    <td class="text-center">
     ...
     <?= $row['date'];?>
      ...
      <td class="text-center">
            <div class="btn-group">
         <a href="child_product.php?edit_product=<?php echo $row['id']; ?>" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>
         <a href="all_child_products.php?delete_product=<?php echo $row['id'];?>" onClick ="return confirm('Are you sure You want to Delete')" class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Remove"><i class="fa fa-times"></i></a>
                                            </div>
                                        </td>
                        </tr>
