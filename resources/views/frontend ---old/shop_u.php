          ...
          //$sql = "SELECT * from product_tb  WHERE id = $edit_product LIMIT 1";
                    ...
                    if(isset($category_slug_sub)){
                        ...
                        if($filter == "low"){
                          ...
                          $limit .= $limit.' ORDER BY '.$order_price.'  ASC LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows;

                          $sql = $limit;

                        }elseif ($filter == "high") {

                          $limit .= $limit.' ORDER BY '.$check_price.$order_price.'  DESC LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows;

                          $sql = $limit;

                        }else{

                         $limit .= $limit.' ORDER BY date DESC LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows;
