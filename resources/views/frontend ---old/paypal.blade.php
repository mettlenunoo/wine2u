...
$item_name = "Item(s) bought From Skingourmet";
...
<form action="<?= $paypal_url;?>cgi-bin/webscr" method="Post" name="buyCredit" id="buyCredit">
  ...
  <input type="hidden" name="currency_code" value="{{ strtoupper($currency) }}">
	...
	<input type="hidden" name="notify_url" value="{{ asset($country.'/thankyou') }}">
  <input type="hidden" name="return" value="{{ asset($country.'/thankyou') }}">
  ...
  <input type="hidden" name="cancel_return" value="{{ asset($country.'/checkout') }}">
